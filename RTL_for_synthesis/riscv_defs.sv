...
// CSR Registers
//--------------------------------------------------------------------
`define CSR_MSTATUS       12'h300   // CPU status (interrupt enable, privilege state)
`define CSR_MSTATUS_MASK  32'hFFFFFFFF

`define CSR_MISA          12'h301   // CPU ISA description (which extensions exist)
`define CSR_MISA_MASK     32'hFFFFFFFF
`define MISA_RV32     32'h40000000 // RV32 architecture
`define MISA_RVI      32'h00000100 // Base integer ISA
`define MISA_RVE      32'h00000010 // Embedded ISA
`define MISA_RVM      32'h00001000 // Multiply/Divide extension
`define MISA_RVA      32'h00000001 // Atomic extension
`define MISA_RVF      32'h00000020 // Floating-point (single)
`define MISA_RVD      32'h00000008 // Floating-point (double)
`define MISA_RVC      32'h00000004 // Compressed instructions
`define MISA_RVS      32'h00040000 // Supervisor mode supported
`define MISA_RVU      32'h00100000 // User mode supported

`define CSR_MEDELEG       12'h302   // Delegate exceptions to lower privilege
`define CSR_MEDELEG_MASK  32'h0000FFFF

`define CSR_MIDELEG       12'h303   // Delegate interrupts to lower privilege
`define CSR_MIDELEG_MASK  32'h0000FFFF

`define CSR_MIE           12'h304   // Interrupt enable bits
`define CSR_MIE_MASK      `IRQ_MASK

`define CSR_MTVEC         12'h305   // Trap/interrupt handler address
`define CSR_MTVEC_MASK    32'hFFFFFFFF

`define CSR_MSCRATCH      12'h340   // Temporary storage for handler
`define CSR_MSCRATCH_MASK 32'hFFFFFFFF

`define CSR_MEPC          12'h341   // Saved PC on trap/interrupt
`define CSR_MEPC_MASK     32'hFFFFFFFF

`define CSR_MCAUSE        12'h342   // Reason for trap/interrupt
`define CSR_MCAUSE_MASK   32'h8000000F

`define CSR_MTVAL         12'h343   // Extra fault information (bad address, etc.)
`define CSR_MTVAL_MASK    32'hFFFFFFFF

`define CSR_MIP           12'h344   // Pending interrupt flags
`define CSR_MIP_MASK      `IRQ_MASK

`define CSR_MCYCLE        12'hc00   // Cycle counter (low)
`define CSR_MCYCLE_MASK   32'hFFFFFFFF

`define CSR_MTIME         12'hc01   // Timer value (low)
`define CSR_MTIME_MASK    32'hFFFFFFFF

`define CSR_MTIMEH        12'hc81   // Timer value (high)
`define CSR_MTIMEH_MASK   32'hFFFFFFFF

`define CSR_MHARTID       12'hF14   // Core (hart) ID
