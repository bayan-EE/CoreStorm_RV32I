...
module tb_riscv_decode_extra;
  ...
  riscv_decode #(
	.SUPPORT_MULDIV(1'b1),
	.EXTRA_DECODE_STAGE(1'b1)   // תחליף ל-1 אם אתה רוצה לבדוק גם extra stage
  ) dut (
	.clk_i(clk),
	.rst_ni(rst_ni),
  ...
  endtask
  task automatic wait_out0_valid(input int max_cycles);
	  int k;
	  begin
		for (k = 0; k < max_cycles; k++) begin
		  @(posedge clk);
		  if (out0_valid_o) return;
		end
		$fatal("TB_FAIL: timeout waiting for out0_valid_o");
	  end
	endtask
  ...
  task automatic test_basic();
	...
	begin
	  ...
	  // wait until outputs appear (EXTRA=0/1)
	  wait_out0_valid(20);
  ...
  task automatic test_stall_stability();
	...
	begin
	  ...
	  // במקום להניח שזה יופיע במחזור הבא:
	  wait_out0_valid(20);
  ...
  task automatic test_flags();
	logic [31:0] base;
	begin
	  $display("[T7] flags correctness");
	  base = 32'h0000_A000;

	  // inst0 = LW (lsu + rd_valid)
	  // inst1 = BEQ (branch, no rd)
	  push_bundle(base, OP_LW, OP_BEQ, 2'b00, 1'b0, 1'b0);


	  wait_out0_valid(20);

	  tb_expect(out0_valid_o, "lane0 valid");
	  tb_expect(out0_instr_lsu_o == 1, "LW must set LSU");
	  tb_expect(out0_instr_rd_valid_o == 1, "LW must set rd_valid");
	  tb_expect(out0_instr_branch_o == 0, "LW is not branch");

	  tb_expect(out1_valid_o, "lane1 valid");
	  tb_expect(out1_instr_branch_o == 1, "BEQ must set BRANCH");
	  tb_expect(out1_instr_rd_valid_o == 0, "BEQ must not set rd_valid");
