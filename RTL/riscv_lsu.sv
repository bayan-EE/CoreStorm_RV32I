...
(
   ...
   ,input           opcode_valid_i        // Current opcode is valid
   ,input  [XLEN-1:0]   opcode_opcode_i      // Raw RISC-V instruction word
   ,input  [XLEN-1:0]   opcode_pc_i          // Program counter of instruction
   ,input           opcode_invalid_i     // Instruction decode marked invalid
   ,input  [4:0]    opcode_rd_idx_i      // Destination register index (rd)
   ,input  [4:0]    opcode_ra_idx_i      // Source register A index (rs1)
   ,input  [4:0]    opcode_rb_idx_i      // Source register B index (rs2)
   ,input  [XLEN-1:0]   opcode_ra_operand_i  // Value of source register rs1
   ,input  [XLEN-1:0]   opcode_rb_operand_i  // Value of source register rs2
   ,input  [XLEN-1:0]   mem_data_rd_i        // Read data returned from memory
   ,input           mem_accept_i         // Memory accepted request this cycle
   ,input           mem_ack_i            // Memory response valid
   ,input           mem_error_i          // Memory access error indication
   ,input  [10:0]   mem_resp_tag_i       // Tag associated with memory response
   ,input           mem_load_fault_i     // Load page fault indication
   ,input           mem_store_fault_i    // Store page fault indication

   // Outputs
   ,output [XLEN-1:0]   mem_addr_o            // Aligned memory address
   ,output [XLEN-1:0]   mem_data_wr_o         // Write data to memory
   ,output          mem_rd_o              // Memory read request
   ,output [3:0]    mem_wr_o              // Byte write enables for store
   ,output          mem_cacheable_o       // Access is cacheable
   ,output [10:0]   mem_req_tag_o         // Tag for memory request
   ,output          mem_invalidate_o      // Cache invalidate command
   ,output          mem_writeback_o       // Cache writeback command
   ,output          mem_flush_o           // Cache flush command
   ,output          writeback_valid_o     // Writeback result is valid
   ,output [XLEN-1:0]   writeback_value_o     // Value to write back to register file
   ,output [5:0]    writeback_exception_o // Exception code for writeback stage
   ,output          stall_o               // Stall pipeline due to LSU
);
...
// Registers / Wires
//-----------------------------------------------------------------
reg [XLEN-1:0]  mem_addr_q;      	 // Latched effective memory address
reg [XLEN-1:0]  mem_data_wr_q;   	 // Latched store write data
reg         	mem_rd_q;            // Latched memory read request
reg [3:0]   	mem_wr_q;            // Latched byte write enables
reg         	mem_cacheable_q;     // Access is cacheable
reg         	mem_invalidate_q;    // Cache invalidate operation
reg         	mem_writeback_q;     // Cache writeback operation
reg         	mem_flush_q;         // Cache flush operation
reg         	mem_unaligned_e1_q;  // Unaligned access detected (E1 stage)
reg         	mem_unaligned_e2_q;  // Unaligned access propagated to E2 stage

reg        	 	mem_load_q;          // Current operation is a load
reg         	mem_xb_q;            // Byte-sized memory access
reg         	mem_xh_q;            // Half-word-sized memory access
reg         	mem_ls_q;            // Signed load (sign extension required)
...
// Opcode decode
//-----------------------------------------------------------------
// Detect if the instruction is any of the load types (signed or unsigned)
...
// Detect signed load instructions only (LB, LH, LW)
wire load_signed_inst_w = (((opcode_opcode_i & `INST_LB_MASK) == `INST_LB)  || 
                           ((opcode_opcode_i & `INST_LH_MASK) == `INST_LH)  || 
                           ((opcode_opcode_i & `INST_LW_MASK) == `INST_LW));

// Detect if the instruction is any store type (SB, SH, SW)
...
// Half-word-aligned access detection (SH, LH, LHU) — useful for 16-bit access width
